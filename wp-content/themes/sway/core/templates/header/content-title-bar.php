<?php
  ...
  if (is_home() && is_front_page()) {
    ...
    // FIX Show/hide breadcrumbs for single pages or the main blog page
    if ( is_home() ) {
        $post_id_breadcrumb = get_option( 'page_for_posts' );
	} else {
		$post_id_breadcrumb = get_the_ID();
	}
  ...
  $themetek_page_showhide_breadcrumbs = get_post_meta( $post_id_breadcrumb, 'keydesign_page_showhide_breadcrumbs', true );
...
<?php elseif ( class_exists( 'Tribe__Events__Main' ) && ( ( tribe_is_event() || tribe_is_event_category() || tribe_is_in_main_loop() || is_post_type_archive( 'tribe_events' ) || 'tribe_events' == get_post_type() || is_singular( 'tribe_events' ) ) ) ) : ?>
     ...
     <div class="row blog-page-heading <?php echo esc_attr($title_align_class); ?>">
        ...
        <div class="container">
          ...
          <?php elseif ( is_search() ) : ?>
            <h1 class="section-heading"><?php echo apply_filters( 'kd_search_query_title', __("Search results for:", "sway") ); ?> <?php the_search_query(); ?></h1>
          <?php elseif ( is_category() ) : ?>
            <h1 class="section-heading"><?php echo apply_filters( 'kd_single_cat_title', __("Currently browsing:", "sway") ); ?> <?php single_cat_title(); ?></h1>
          <?php elseif ( is_tag() ) : ?>
            <h1 class="section-heading"><?php echo apply_filters( 'kd_single_tag_title', __("All posts tagged:", "sway") ); ?> <?php single_tag_title(); ?></h1>
          <?php elseif ( is_author() ) : ?>
            <h1 class="section-heading"><?php echo apply_filters( 'kd_author_archive_title', __("All posts by", "sway") ); ?> <?php echo esc_html( get_userdata( get_query_var( 'author' ) )->display_name ); ?></h1>
            <?php echo wp_kses_post( wpautop( get_the_author_meta( 'description' ) ) ); ?>
          <?php elseif ( is_day() ) : ?>
            <h1 class="section-heading"><?php echo apply_filters( 'kd_day_archive_title', __("Posts archive for", "sway") ); ?> <?php echo get_the_date('F jS, Y'); ?></h1>
          <?php elseif ( is_month() ) : ?>
            <h1 class="section-heading"><?php echo apply_filters( 'kd_month_archive_title', __("Posts archive for", "sway") ); ?> <?php echo get_the_date('F, Y'); ?></h1>
          <?php elseif ( is_year() ) : ?>
            <h1 class="section-heading"><?php echo apply_filters( 'kd_year_archive_title', __("Posts archive for", "sway") ); ?> <?php echo get_the_date('Y'); ?></h1>
