    ...
    class KD_ELEM_MAP extends KEYDESIGN_ADDON_CLASS {
                ...
                vc_map(array(
                    ...
                    "params" => array(
                        array(
                          ...
                          "text" => "<span style='display: block;'>Google maps requires a valid API key in order to work. You can add it in Dashboard > Sway > Theme Options > Global Options. You can generate a new API key from <a href='https://developers.google.com/maps/faq#new-key' target='_blank' title='Google Maps API'>here</a>.</span>",
                        ...
                        array(
                            ...
                            "group"         => esc_html__("Business info", "keydesign"),
            								"value"			=>	array(
                                esc_html__( "Disable", "keydesign" ) => "hidden_info_panel",
                        ...
                        array(
                            "type"          => "textfield",
                            "class"         => "",
                            "heading"       => esc_html__("Address section title", "keydesign"),
                        ...
                        array(
                            "type"          => "textarea",
                            "class"         => "",
                            "heading"       => esc_html__("Address section description", "keydesign"),
                            "param_name"    => "map_business_address_description",
                            "value"         => "",
                            "group"         => esc_html__("Business info", "keydesign"),
                            "description"   => esc_html__("Enter address section description.", "keydesign"),
                            "dependency" =>	array(
                                "element" => "map_business_panel_settings",
                                "value" => array("left_info_panel", "right_info_panel")
                            ),
                        ...
                        array(
                            "type"          => "textfield",
                            "class"         => "",
                            "heading"       => esc_html__("Phone section title", "keydesign"),
                        ...
                        array(
                            "type"          => "textarea",
                            "class"         => "",
                            "heading"       => esc_html__("Phone section description", "keydesign"),
                            "param_name"    => "map_business_phone_description",
                            "value"         => "",
                            "group"         => esc_html__("Business info", "keydesign"),
                            "description"   => esc_html__("Enter phone section description.", "keydesign"),
                            "dependency" =>	array(
                                "element" => "map_business_panel_settings",
                                "value" => array("left_info_panel", "right_info_panel")
                            ),
                        ...
                        array(
                            "type"          => "textfield",
                            "class"         => "",
                            "heading"       => esc_html__("Opening hours section title", "keydesign"),
        ...
        public function kd_map_shrt($atts, $content = null) {
            ...
            extract(shortcode_atts(array(
                ...
                'map_business_address_description' => '',
                'map_business_phone' => '',
                'map_business_phone_description' => '',
            ...
            if (isset($redux_ThemeTek['tek-google-api']) && $redux_ThemeTek['tek-google-api'] != '' ) {
      			$output .= '<script>
                    ...
                    async function initKdMap_'.$id.'() {
                      ...
                      marker_image.src = "'.$content_image.'";
      								const marker_'.$id.' = new AdvancedMarkerElement({
      									map: map_'.$id.',
      									draggable: false,';
                        if ( $content_image != '' ) {
                        $output .= 'content: marker_image,';
                        }
      									$output .= 'position: gmap_location_'.$id.',
                      });
                      var viewportwidth;
                      ...
                      if ( $map_business_panel_settings == 'right_info_panel' ) {
                        $output .= 'if ( viewportwidth >= 960 ) {
                          map_'.$id.'.panBy(300, 0);
                        }';
                      }

                      if ( $map_business_panel_settings == 'left_info_panel' ) {
                        $output .= 'if ( viewportwidth >= 960 ) {
                          map_'.$id.'.panBy(-300, 0);
                        }';
                      }
            ...
            $output .= '<div class="kd-alertbox kd-contact-alertbox ab_info kd_map_alert">
                        ...
                        <p>Google maps requires a valid API key in order to work.<br>You can add it in Dashboard > Sway > Theme Options > Global Options. Generate a new API key from https://developers.google.com/maps</p></div>
                ...
                if ($map_business_panel_settings != "hidden_info_panel") {
                    $output .= '<div class="business-info-wrapper">
                        <span class="fas fa-times toggle-map-info"></span>
                          <div class="business-address-row"><i class="fas sway-map-pin"></i>';
                            if ( $map_business_address != '' ) {
                              $output .= '<h4 class="map-address-title">'.$map_business_address.'</h4>';
                            }
                            if ( $map_business_address_description != '' ) {
                              $output .= '<p class="map-address-description">'.$map_business_address_description.'</p>';
                            }
                          $output .= '</div>
                          <div class="business-phone-row"><i class="fas sway-phone-topbar"></i>';
                            if ( $map_business_phone != '' ) {
                                $output .= '<h4 class="map-phone-title">'.$map_business_phone.'</h4>';
                            }
                            if ( $map_business_phone_description != '' ) {
                              $output .= '<p class="map-phone-description">'.$map_business_phone_description.'</p>';
                            }
                          $output .= '</div>
                          <div class="business-opening-hours"><i class="far fa-clock"></i>';
                          if ( $map_business_opening_hours != '' ) {
                              $output .= '<h4 class="map-opening-hours-title">'.$map_business_opening_hours.'</h4>';
                          }

                          $output .= '<div class="business-info-schedule">';
