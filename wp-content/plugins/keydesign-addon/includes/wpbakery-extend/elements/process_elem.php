    ...
    class tek_process extends KEYDESIGN_ADDON_CLASS
    {
                ...
                vc_map(array(
                    ...
                    "params" => array(
                        ...
                        array(
                            ...
                            "save_always" => true,
                            "description" => esc_html__("If icon is selected, the step number will be hidden.", "keydesign"),
                            "group" => esc_html__( "Content", "keydesign" ),
                        ),
                        array(
                            "type" => "textfield",
                        ...
                        array(
                             ...
                             "value" =>	array(
                                  ...
                                  esc_html__( 'Heading link', 'keydesign' ) => 'heading-link',
                              ),
                             "save_always" => true,
                        ...
                        array(
                            "type" => "dropdown",
                            "class" => "",
                            "heading" => esc_html__("Link action", "keydesign"),
                            ...
                            "dependency" => array(
                               ...
                               "value"  => array( "1", "text-link", "heading-link" ),
                           ),
                           "group" => esc_html__( "Link", "keydesign" ),
                        ),
                        array(
                        ...
                        array(
                            "type" => "colorpicker",
                            "class" => "",
                            "heading" => esc_html__("Text link color", "keydesign"),
                            "param_name" => "pss_text_link_color",
                            "value" => "",
                            "description" => esc_html__("Select text link color. If none selected, the default theme color will be used.", "keydesign"),
                            "dependency" => array(
                               "element" => "pss_custom_link",
                        ...
                        array(
                            "type" => "dropdown",
                            "class" => "",
                            "heading" => esc_html__("Button hover style", "keydesign"),
                            "param_name" => "pss_button_hover_style",
                            "value" => array(
                            ...
                            "save_always" => true,
                            "description" => esc_html__("Select button style on hover.", "keydesign"),
                            "group" => esc_html__( "Link", "keydesign" ),
                        ),
                        array(
                            "type" => "dropdown",
        ...
        public function kd_process_single($atts, $content = null) {
            extract(shortcode_atts(array(
                ...
                'pss_button_hover_style' => '',
                'css_animation' => '',
                'elem_animation_delay' => '',
                'pss_extra_class' => '',
            ), $atts));

            $content_icon = $icon_color_style = $icon_size_style = $link_title = $link_target = $pss_icon = $animation_delay = '';
            $heading_anchor_before = $heading_anchor_after = '';

            if ( $pss_icon_type == 'icon_browser' && strlen( $icon_iconsmind ) > 0 ) {
            ...
            if ( $pss_icon_type == 'icon_browser' && !empty($pss_icon) ) {
  				$content_icon = '<div class="process-icon"><i class="'.$pss_icon .'" style="'.$icon_size_style.' '.$icon_color_style.'"></i></div>';
  			}
            ...
            if ( $pss_custom_link == "heading-link" ) {
                if ( $button_action == "modal-trigger-btn" ) {
                    $heading_anchor_before = '<a class="pointer-effect" data-toggle="modal" data-target="#popup-modal">';
                } elseif ( $button_action == "panel-trigger-btn" ) {
                    $heading_anchor_before = '<a class="pointer-effect '.$button_action.'">';
                } else {
                    $heading_anchor_before = '<a href="'.$href['url'].'" '.$link_target.'>';
                }
                $heading_anchor_after = '</a>';
            }
            ...
            $output = '
                <li class="pss-item '.$css_animation.' '.$pss_extra_class.'" '.$animation_delay.'>
                    <div class="pss-container">';
                    if ( $pss_display_number == "process_number_on" && $pss_icon_type == 'no_icon' ) {
                    ...
                    if ( $pss_icon_type != 'no_icon' ) {
                        $output .= '<div class="pss-step-number"><span>'.$content_icon.'</span></div>';
                    }
                        $output .= '<div class="pss-text-area">';
                            if ( '' != $pss_title ) {
                                $output .= $heading_anchor_before;
                                $output .= '<h5 '.(!empty($pss_title_color) ? 'style="color: '.$pss_title_color.';"' : '').'>'.$pss_title.'</h5>';
                                $output .= $heading_anchor_after;
                            }
                            if ( '' != $pss_description ) {
                                $output .= '<p '.(!empty($pss_description_color) ? 'style="color: '.$pss_description_color.';"' : '').'>'.$pss_description.'</p>';
                            }
                            if ( $pss_custom_link == "1" ) {
                              if ( $button_action == "modal-trigger-btn" ) {
                                $output .= '<a class="tt_button '.$pss_button_style.' '.$pss_button_color_scheme.' '.$pss_button_hover_style.'" data-toggle="modal" data-target="#popup-modal">'.$pss_link_text.'</a>';
                              } elseif ( $button_action == "panel-trigger-btn" ) {
                                $output .= '<a class="tt_button '.$pss_button_style.' '.$pss_button_color_scheme.' '.$pss_button_hover_style.' '.$button_action.'">'.$pss_link_text.'</a>';
                              } else {
                                $output .= '<a href="'.$href['url'].'" class="tt_button '.$pss_button_style.' '.$pss_button_color_scheme.' '.$pss_button_hover_style.'" '.$link_target.''.$link_title.'>'.$pss_link_text.'</a>';
                              }
                            } elseif ( $pss_custom_link == "text-link" ) {
                              if ( $button_action == "modal-trigger-btn" ) {
                                $output .= '<a class="process-text-link" data-toggle="modal" data-target="#popup-modal" '.(!empty($pss_text_link_color) ? 'style="color: '.$pss_text_link_color.';"' : '').'>'.$pss_link_text.'</a>';
                              } elseif ( $button_action == "panel-trigger-btn" ) {
                                $output .= '<a class="process-text-link '.$button_action.'" '.(!empty($pss_text_link_color) ? 'style="color: '.$pss_text_link_color.';"' : '').'>'.$pss_link_text.'</a>';
                              } else {
                                $output .= '<a class="process-text-link" href="'.$href['url'].'" '.$link_target.''.$link_title.' '.(!empty($pss_text_link_color) ? 'style="color: '.$pss_text_link_color.';"' : '').'>'.$pss_link_text.'</a>';
                              }
                            }
                        $output .= '</div>
                    </div>
