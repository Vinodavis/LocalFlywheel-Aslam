...
if (!class_exists('tek_reviewcarousel')) {
    class tek_reviewcarousel extends KEYDESIGN_ADDON_CLASS
    {
                ...
                vc_map(array(
                    ...
                    "params" => array(
                        ...
                        array(
                            ...
                            "value" => "",
                            "description" => esc_html__("Write the review title.", "keydesign")
                        ),
                        ...
                        array(
                            ...
                            "value" => "",
                            "description" => esc_html__("Select review title text color. If none selected, the default theme color will be used.", "keydesign"),
                        ),
                        ...
                        array(
                            ...
                            "param_name" => "rw_review_message_color",
                            "value" => "",
                            "description" => esc_html__("Select review message text color. If none selected, the default theme color will be used.", "keydesign"),
                        ),
        ...
        public function kd_reviewcarousel_container($atts, $content = null) {
            extract(shortcode_atts(array(
              ...
              'rwp_extra_class' => '',
                ...
                $wrapper_class = implode(' ', array('review-carousel', 'rc-parent', $kd_rwpunique_id, $rwp_navigation_color, $rwp_nav_style, $rwp_loop, $rwp_extra_class));
                ...
                $output .= '<script>
                    ...
                    if ($(".review-carousel.'.$kd_rwpunique_id.' .rc-content").length) {
                      $(".review-carousel.'.$kd_rwpunique_id.' .rc-content").owlCarousel({
                        ...
                        if ( $rwp_loop != "loop_off" && ! vc_is_inline() ) {
                          $output .= 'loop: true,';
                        } else {
        ...
        public function kd_reviewcarousel_single($atts, $content = null) {
            ...
            $author_img = $full_star = $empty_star = $rating_stars = $rating_stars_text = $rw_author_img_array = $author_image = $animation_delay = $dimensions = $hwstring = '';

            $full_star = '<span class="sway-full-star" '.(!empty($rw_star_color) ? 'style="color: '.$rw_star_color.';"' : '').'></span>';
            $empty_star = '<span class="sway-empty-star" '.(!empty($rw_star_color) ? 'style="color: '.$rw_star_color.';"' : '').'></span>';
            ...
            if ($image_source == 'external_link') {
              if (!empty($ext_image)) {
                $author_image .='<img src="'.$ext_image.'" alt="" '.$hwstring.' />';
              }
            ...
            $output = '
                <div class="key-reviews '.$css_animation.' '.$rw_extra_class.'" '.(!empty($rw_box_background) ? 'style="background-color: '.$rw_box_background.';"' : '').' '.$animation_delay.'>';
                    ...
                    if (  $rw_review_title ) {
                      $output .= '<h4 class="rw_title" '.(!empty($rw_review_title_color) ? 'style="color: '.$rw_review_title_color.';"' : '').'>'.$rw_review_title.'</h4>';
                    }
                    ...
                    if ( $rw_show_review_rating !== "hide_stars" ) {
                      $output .= '<div class="rw_rating"><span class="rating_intro" '.(!empty($rw_star_color) ? 'style="color: '.$rw_star_color.';"' : '').'>'.$rating_stars_text.'.0</span>
                         '.$rating_stars.'
                      </div>';
                    }
                $output .= '</div>';
