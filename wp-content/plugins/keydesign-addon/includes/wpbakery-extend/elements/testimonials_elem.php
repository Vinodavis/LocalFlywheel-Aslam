...
if (!class_exists('tek_testimonials')) {
    class tek_testimonials extends KEYDESIGN_ADDON_CLASS
    {
                ...
                vc_map(array(
                    ...
                    "params" => array(
                        ...
                        array(
                          "type" => "dropdown",
                          "class" => "",
                          "heading" => esc_html__("Title HTML tag", "keydesign"),
                          "param_name" => "tt_heading_tag",
                          "value" => array(
                              "Default" => "",
                              "h1" => "h1",
                              "h2" => "h2",
                          ...
                          "save_always" => true,
                          "description" => esc_html__("Select title HTML tag. The default is h5.", "keydesign")
                        ),

                        array(
                            "type" => "textarea",
                            "heading" => esc_html__("Testimonial quote", "keydesign"),
                            "param_name" => "tt_quote",
                        ),
                        ...
                        array(
                          "type" => "dropdown",
                          "class" => "",
                          "heading" => esc_html__("Quote HTML tag", "keydesign"),
                          "param_name" => "tt_quote_tag",
                          "value" => array(
                              "Default" => "",
                              "h1" => "h1",
                              "h2" => "h2",
                          ...
                          "save_always" => true,
                          "description" => esc_html__("Select quote HTML tag. The default is h6.", "keydesign")
                        ),
                        ...
                        array(
                          "type" => "checkbox",
                          "heading" => esc_html__( "Enable link on author name text?", "keydesign" ),
                          "param_name" => "author_name_link",
        ...
        public function kd_testimonials_single($atts, $content = null) {
            ...
            // Author name link settings
            if ( !empty( $author_name_link ) && $author_link_url != '' ) {
                $link_before = '<a href="' . $author_link_url . '" target="' . $author_link_target . '">';
                $link_after = '</a>';
            }
              ...
              <div class="tt-content-inner">';
                ...
                $output .= '<div class="tt-container" '.(!empty($tt_bg_style) ? 'style="'.$tt_bg_style.'"' : '').'>';
                    if ( $GLOBALS['global_image_layout'] == 'without-image' ) {
                      if ( $tt_quote ) {
                        if ( $tt_quote_tag != '' ) {
                          $output .= '<'.esc_attr( $tt_quote_tag ) . ' class="testimonial-quote" ' . ( !empty( $tt_text_style ) ? 'style="'.$tt_text_style.'"' : '') . '>' . $tt_quote . '</'.esc_attr( $tt_quote_tag ).'>';
                        } else {
                          $output .= '<h6 class="testimonial-quote" '.(!empty($tt_text_style) ? 'style="'.$tt_text_style.'"' : '').'>'.$tt_quote.'</h6>';
                        }
                      }
                    } elseif ( $GLOBALS['global_image_layout'] == 'with-image' ) {
                      if ( $tt_heading ) {
                        if ( $tt_heading_tag != '' ) {
                          $output .= '<'.esc_attr( $tt_heading_tag ) . ' class="testimonial-title" ' . ( !empty( $tt_text_style ) ? 'style="'.$tt_text_style.'"' : '') . '>' . $tt_heading . '</'.esc_attr( $tt_heading_tag ).'>';
                        } else {
                          $output .= '<h5 class="testimonial-title" '.( !empty( $tt_text_style ) ? 'style="'.$tt_text_style.'"' : '').'>' . $tt_heading . '</h5>';
                        }
                      }
                      if ( $tt_quote ) {
                    ...
                    $output .= '<span class="author" '.(!empty($tt_text_color) ? 'style="color: ' . $tt_text_color . ';"' : '').'>';
                    if ( '' != $tt_title ) {
                        $output .= $link_before;
                        $output .= '<span class="author-name">'.$tt_title.'</span>';
                        $output .= $link_after;
                    }
                    $output .= '<span class="content" '.(!empty($tt_text_color) ? 'style="color: ' . $tt_text_color . ';"' : '').'>'.$tt_position.'</span>
                    </span>
                </div>';
