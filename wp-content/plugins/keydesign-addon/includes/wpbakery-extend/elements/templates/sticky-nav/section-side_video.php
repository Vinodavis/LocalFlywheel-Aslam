  ...
  function kd_section_set_side_video($atts,$content = null){
    ...
    if ( $video_image_source == 'external_link' ) {
        if (!$video_ext_image) {
            ...
            $content_image ='<img src="'.$video_ext_image.'" alt="" '.$hwstring.' />';
        }
    } else {
        if (!$fss_video_cover) {
            $content_image ='<img src="'.$default_src.'" alt="" class="vc_img-placeholder" />';
        } else {
            if ( '' != $fss_video_cover && wp_attachment_is_image( $fss_video_cover ) ) {
    ...
    $output = '<div class="' . trim($wrapper_class) . '" ' . $animation_delay . '>
      <div class="entry-video">
        <div class="video-cover">
          ...
          <div class="play-video kd-animate '.$fss_play_button.'"><span class="sway-play"></span></div>
          '.$embed.'
        </div>
      </div>
    </div>';
