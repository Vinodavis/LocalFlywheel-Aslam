...
function keydesign_vc_after_init_actions() {
  ...
  if ( function_exists('vc_add_param') ) {
      ...
      // Add parameters to vc_column and vc_column_inner

      $attributes_column = array(
        array(
          'type' => 'checkbox',
          'heading' => esc_html__( 'Sticky column', 'keydesign' ),
          'param_name' => 'kd_sticky_column',
          'description' => esc_html__( 'If checked the column will take a sticky position and scroll with the content.', 'keydesign' ),
          'weight' => 1,
        ),
      );

      foreach ($attributes_column as $attribute) {
        vc_add_param( 'vc_column', $attribute );
        vc_add_param( 'vc_column_inner', $attribute );
      }
...
// Overwrite VC Grid default templates
function keydesign_vc_grid_default_templates() {
    $src = include plugin_dir_path( __FILE__ ) . 'vc-elements/vc_grid_item/templates.php';
    return $src;
}
add_filter( 'vc_grid_item_predefined_templates', 'keydesign_vc_grid_default_templates' );
